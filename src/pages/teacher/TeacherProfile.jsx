...
/* ─── shared design-token stylesheet — identical id/tokens to the
   rest of the app (same as StudentProfile); a no-op if already
   mounted by the layout or another page. ─── */
const injectStyles = () => {
...
/* ─── glass-badge stylesheet — identical to StudentProfile's, used
   for the subject / class-teacher badge on the ID card front. ─── */
...
/* ─── ID-card stylesheet — identical structure to StudentProfile's,
   reused here for the staff badge (same CR80 proportions, real 3D
   flip, hologram seal, light-sweep on hover). ─── */
...
export default function TeacherProfile() {
  ...
  // Refresh from the server on mount so photoUrl and every other
  // field reflects the latest data (localStorage's copy is only as
  ...
  const roleLabel = user.isClassTeacher ? "Class Teacher" : "Subject Teacher";
  ...
  return (
    <main className="dash-main" style={D.main}>
      ...
      <div className="profile-two-col" style={D.twoCol}>
        {/* Account Details — flippable Staff ID card + full record */}
            ...
            <div>
                  ...
                  <div className="id-card-face id-card-front">
                    ...
                    <div style={idFrontStyles.contentWrap}>
                      ...
                      <span style={idFrontStyles.role} className="glass-badge">
                        {user.isClassTeacher ? "🎓" : "📘"} {roleLabel}
                      </span>

                      <div style={idFrontStyles.detailsBox}>
                        <div style={idFrontStyles.detailRow}>
                          <span style={idFrontStyles.detailLabel}>STAFF ID</span>
                          <span style={idFrontStyles.detailValue}>{user.staffId || user.username || "N/A"}</span>
                        </div>
                        <div style={idFrontStyles.detailRow}>
                    ...
                    <div style={idFrontStyles.contentWrap}>
                      ...
                      <div style={idBackStyles.infoStack}>
                        <div style={idBackStyles.metaLine}>
                          ...
                          <span style={idBackStyles.detailValueSmall}>{user.phone || "N/A"}</span>
                        </div>
                        <div style={idBackStyles.metaLine}>
                          <span style={idBackStyles.detailLabel}>ROLE</span>
                          <span style={idBackStyles.detailValueSmall}>{user.role || "Teacher"}</span>
                        </div>
                      </div>

                      <div style={idBackStyles.barcodeCard}>
                        <div style={idBackStyles.barcode} />
                        <span style={idBackStyles.barcodeNum}>{(user.staffId || user.username || "N/A").toUpperCase()}</span>
                      </div>
              ...
              <div style={{ ...D.photoBlock, marginTop: 18 }}>
                ...
                <div>
                  ...
                  <p style={D.photoHint}>
                    {user.photoUrl ? "Tap the camera icon to replace it — this also updates your ID card above." : "No photo on file yet — required for your staff ID."}
                  </p>
                  {photoMsg && (
              ...
              {/* Full record — every field on file for this teacher */}
              <div style={{ marginTop: 4 }}>
                ...
                <div style={D.infoRow}>
                  <span style={D.infoLabel}>Staff ID</span>
                  <span style={D.infoValue}>{user.staffId || user.username || "—"}</span>
                </div>
                <div style={D.infoRow}>
                  <span style={D.infoLabel}>Subject</span>
                  <span style={D.infoValue}>{user.subject || "—"}</span>
                </div>
                <div style={D.infoRow}>
                  <span style={D.infoLabel}>Position</span>
                  <span style={D.infoValue}>{roleLabel}</span>
                </div>
                <div style={D.infoRow}>
                ...
                <div style={{ ...D.infoRow, borderBottom: "none" }}>
                  ...
                  <span style={D.infoValue}>{user.role || "Teacher"}</span>
                </div>
              </div>
            </div>
          )}
        ...
        <section style={D.panel} aria-label="Change password">
          ...
          <button
            ...
            style={{ ...D.button, opacity: saving || !oldPass || !newPass ? 0.6 : 1 }}
          >
...
/* ===== ID CARD — front face (portrait badge layout) — identical
   structure to StudentProfile's, "role" here is Class/Subject
   teacher instead of a class name. ===== */
const idFrontStyles = {
  contentWrap: {
    position: "relative",
    zIndex: 3,
