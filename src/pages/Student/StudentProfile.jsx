...
export default function StudentProfile() {
  ...
  // classes available to pick from — same Classes table / endpoint the
  // admin e-assessments screens already use, so a student can only
  // select a class that genuinely exists rather than typing anything.
  ...
  /* ================= LOAD CLASSES =================
     Same endpoint the admin's e-assessments/analytics screens use
     (GET /e-assessments/classes → real rows from the Classes table),
     so the dropdown always reflects whatever classes the school has
     actually set up rather than whatever a student happens to type. */
  const loadClasses = async () => {
    try {
      setClassesLoading(true);
      setClassesError(false);
      const res = await API.get("/e-assessments/classes");
      setClasses(res.data || []);
    } catch (err) {
      console.log(err);
  ...
  // If the student's current class isn't in the fetched list (e.g. an
  // older free-text value, or the class was since renamed/removed),
  // keep it selectable as its own option instead of silently dropping
  // it from the field the moment the page loads.
  const classOptions = (() => {
    const names = classes.map((c) => c.name ?? c.class_name).filter(Boolean);
    if (studentClass && !names.includes(studentClass)) {
      return [studentClass, ...names];
    }
    return names;
  })();
      ...
      <div className="profile-two-col" style={D.twoCol}>
        ...
        <section style={D.panel} aria-label="Profile details">
          ...
          <div style={D.formGroup}>
            ...
            {classesError && (
              <p style={D.fieldHint}>
                Couldn't load the class list — try refreshing the page.{studentClass ? " Your current class is kept selected below." : ""}
              </p>
            )}
            {!classesLoading && !classesError && classes.length === 0 && (
              <p style={D.fieldHint}>No classes have been set up yet — contact your administrator.</p>
            )}
          </div>
